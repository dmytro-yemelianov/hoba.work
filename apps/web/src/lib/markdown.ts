...
function renderSpecimen(specimen: Specimen, t: Translate): string[] {
  const out = [
    `**${specimen.label}**${specimen.context ? ` · ${specimen.context}` : ''} — *${t('specimen.badge')}*`,
    '',
  ];
  out.push('```');
  if (specimen.subject)
    out.push(`${asHeading(t('specimen.subject.line'))}: ${specimen.subject}`, '');
...
export function entityMarkdown(
  ...
  const summary =
    'summary' in entity ? entity.summary : 'description' in entity ? entity.description : '';
  ...
  if ('stages' in entity)
    facts.push(
      `${asHeading(t('common.stage'))}: ${entity.stages.map((s) => t(`stage.${s}`)).join(', ')}`
    );
  if ('stage' in entity)
    facts.push(`${asHeading(t('common.stage'))}: ${t(`stage.${entity.stage}`)}`);
  ...
  if ('facets' in entity) {
    ...
    facts.push(
      `${asHeading(t('facet.visibility'))}: ${t(`visibility.${entity.facets.visibility}`)}`
    );
    facts.push(
      `${asHeading(t('facet.removability'))}: ${t(`removability.short.${entity.facets.removability}`)}`
    );
    if (entity.honest_baseline) facts.push(t('mech.honest'));
  }
  if ('actor' in entity && typeof entity.actor === 'string')
    facts.push(`${asHeading(t('int.actor'))}: ${t(`iactor.${entity.actor}`)}`);
  ...
  if ('evidence_level' in entity)
    facts.push(`${asHeading(t('common.evidence'))}: ${t(`level.${entity.evidence_level}`)}`);
  out.push(...bullets(t('md.facts'), facts));

  if ('pass_condition' in entity)
    out.push(`## ${asHeading(t('bar.pass'))}`, '', entity.pass_condition, '');
  if ('trigger_rule' in entity)
    out.push(`## ${asHeading(t('patterns.trigger'))}`, '', entity.trigger_rule, '');
  ...
  if ('establishes' in entity)
    out.push(...bullets(asHeading(t('pat.establishes')), entity.establishes));
  if ('non_inferences' in entity)
    out.push(...bullets(asHeading(t('art.nonInf')), entity.non_inferences));
  if ('expected_effects' in entity)
    out.push(...bullets(asHeading(t('int.effects')), entity.expected_effects));
  if ('measurements' in entity)
    out.push(
      ...bullets(
        asHeading(t('int.measurements')),
        entity.measurements.map((m) => `\`${m}\``)
      )
    );
  ...
  if ('probes' in entity && entity.probes.length) {
    out.push(
      ...bullets(
        t('art.probes'),
        entity.probes.map(
          (p) => `**${p.id}** (${t(`cost.${p.cost}`)}) — ${p.action} → ${p.expected_signal}`
        )
      )
    );
  }
  ...
  if ('operates_at' in entity)
    relations.push(
      ...entity.operates_at.map((id) => `${t('relation.operates_at')} — ${idLink(id, bundle)}`)
    );
  if ('emissions' in entity)
    relations.push(
      ...entity.emissions.map((e) => `${t('relation.emits')} — ${idLink(e.artifact, bundle)}`)
    );
  if ('amplifies' in entity)
    relations.push(
      ...entity.amplifies.map((id) => `${t('relation.amplifies')} — ${idLink(id, bundle)}`)
    );
  if ('masks' in entity)
    relations.push(...entity.masks.map((id) => `${t('relation.masks')} — ${idLink(id, bundle)}`));
  if ('precedes' in entity)
    relations.push(
      ...entity.precedes.map((id) => `${t('relation.precedes')} — ${idLink(id, bundle)}`)
    );
  if ('required_artifacts' in entity)
    relations.push(...entity.required_artifacts.map((id) => idLink(id, bundle)));
  if ('compatible_mechanisms' in entity)
    relations.push(...entity.compatible_mechanisms.map((id) => idLink(id, bundle)));
  if ('mechanisms' in entity && Array.isArray(entity.mechanisms))
    relations.push(...entity.mechanisms.map((id) => idLink(id, bundle)));
  if ('targets' in entity)
    relations.push(
      ...entity.targets.map((id) => `${t('relation.targets')} — ${idLink(id, bundle)}`)
    );
  ...
  if ('edges' in entity && Array.isArray(entity.edges)) {
    out.push(
      ...bullets(
        t('loop.edges'),
        entity.edges.map((e) => `${e.from} ${t(`relation.${e.relation}`)} ${e.to}`)
      )
    );
  }
...
export function catalogueMarkdown(bundle: RegistryBundle, lang: Lang, t: Translate): string {
  ...
  for (const [heading, nodes] of sections) {
    ...
    for (const node of nodes) {
      const summary =
        'summary' in node ? node.summary : 'description' in node ? node.description : '';
      out.push(
        `- [${node.id}](${SITE}/${ROUTES[node.type]}/${node.id}) **${node.title}** — ${summary}`
      );
    }
    out.push('');
  }
