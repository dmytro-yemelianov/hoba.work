  ...
  switch (dna.pose) {
    case 'sitting': {
      ...
      return {
        ...
        patternSpots: [[w(192), 312], [w(308), 312], [w(205), 366], [w(295), 366]],
        paws: [[w(218), 402], [w(282), 402]],
        propLeft: [96, 402],
        propRight: [404, 402],
        clip,
        body:
    ...
    case 'loaf': {
      ...
      return {
        ...
        patternSpots: [[w(182), 330], [w(318), 330], [w(215), 370], [w(285), 370]],
        paws: [[w(202), 400], [w(298), 400]],
        propLeft: [90, 404],
        propRight: [410, 404],
        clip,
        body:
    ...
    case 'orb': {
      ...
      return {
        ...
        patternSpots: [[r1(250 - r * 0.55), 300], [r1(250 + r * 0.55), 300], [250, r1(305 + r * 0.5)]],
        paws: [[220, r1(305 + r * 0.86)], [280, r1(305 + r * 0.86)]],
        ...
        body:
          ...
          beans(220, r1(305 + r * 0.82), 0.95) + beans(280, r1(305 + r * 0.82), 0.95),
      };
    }
    ...
    case 'longcat': {
      ...
      return {
        ...
        patternSpots: [[w(220), 285], [w(280), 322], [w(225), 360]],
        paws: [[w(222), 402], [w(278), 402]],
        propLeft: [108, 404],
        propRight: [392, 404],
        clip,
        body:
          ...
          paw(w(222), 402, 22, 12) + paw(w(278), 402, 22, 12),
      };
    }
    ...
    case 'chilling': {
      ...
      return {
        ...
        patternSpots: [[245, 312], [325, 316], [285, 368]],
        paws: [[212, 394], [352, 394]],
        propLeft: [66, 400],
        propRight: [434, 400],
        clip,
        body:
          ...
          beans(331, 258, 0.82) +
          paw(212, 394, 23, 12) + paw(352, 394, 20, 11),
      };
    }
    ...
    case 'pounce': {
      ...
      return {
        ...
        patternSpots: [[300, 334], [350, 342], [255, 348]],
        paws: [[150, 396], [216, 396]],
        propLeft: [84, 400],
        propRight: [432, 396],
        clip,
        body:
          `<path d="${clip}" ${outline}/>` +
          paw(150, 396, 25, 12) + paw(216, 396, 25, 12),
      };
    }
    ...
    case 'stretching': {
      ...
      return {
        ...
        patternSpots: [[300, 322], [345, 332], [252, 344]],
        paws: [[128, 392], [188, 390]],
        propLeft: [72, 398],
        propRight: [434, 398],
        clip,
        body:
          `<path d="${clip}" ${outline}/>` +
          paw(128, 392, 28, 11) + paw(188, 390, 26, 11) + paw(352, 392, 24, 12),
      };
    }
...
function renderCoatOverlays(dna: CatDNA, colors: CatColors, layout: PoseLayout, prefix: string): string {
  ...
  switch (dna.furPattern) {
    case 'tabbyStripes':
      for (const [x, y] of layout.patternSpots.slice(0, 4)) {
        parts.push(`<path d="M ${x - 26} ${y} C ${x - 10} ${y - 15} ${x + 10} ${y - 15} ${x + 26} ${y} C ${x + 10} ${y - 5} ${x - 10} ${y - 5} ${x - 26} ${y} Z" fill="${colors.secondary}" opacity="0.55"/>`);
      }
      break;
    ...
    case 'dappledSpots':
      for (const [x, y] of layout.patternSpots) {
        ...
        parts.push(`<circle cx="${jx}" cy="${jy}" r="${r1(8 + rng() * 6)}" fill="${colors.secondary}" opacity="0.5"/>`);
        if (rng() > 0.5) parts.push(`<circle cx="${r1(jx + 18)}" cy="${r1(jy + 12)}" r="${r1(4 + rng() * 4)}" fill="${colors.secondary}" opacity="0.4"/>`);
      }
      break;

    case 'heartPatch': {
      const [x, y] = layout.patternSpots[0] ?? [300, 320];
      parts.push(`<path d="M ${x} ${y} C ${x} ${y - 13} ${x - 17} ${y - 13} ${x - 17} ${y} C ${x - 17} ${y + 12} ${x} ${y + 24} ${x} ${y + 24} C ${x} ${y + 24} ${x + 17} ${y + 12} ${x + 17} ${y} C ${x + 17} ${y - 13} ${x} ${y - 13} ${x} ${y} Z" fill="${accent}" opacity="0.92"/>`);
      break;
    }
    ...
    case 'socksAndBib': {
      ...
      parts.push(`<ellipse cx="${bx}" cy="${by}" rx="${brx}" ry="42" fill="${colors.belly === '#ffffff' ? '#ffffff' : shiftTone(colors.belly, 10)}" opacity="0.95"/>`);
      break;
    }
  ...
  const clipped = parts.length > 0 ? `<g class="fur-patterns" clip-path="url(#${prefix}-bodyclip)">${parts.join('')}</g>` : '';

  // Socks sit on the paw shapes, which are outside the body clip.
  let socks = '';
  if (dna.furPattern === 'socksAndBib' && layout.paws.length > 0) {
    socks = `<g class="fur-socks">` + layout.paws.map(([x, y]) =>
...
function renderTail(dna: CatDNA, colors: CatColors, layout: PoseLayout): string {
  ...
  switch (dna.tailType) {
    case 'fluffyPlume':
      ...
      break;
    case 'curlySpiral':
      inner = tube('M 0 0 C 42 -2 70 -26 68 -58 C 66 -90 36 -98 26 -78 C 18 -62 38 -54 46 -66', 22, 15);
      break;
    case 'bobtailBun':
      inner = `<circle cx="8" cy="-12" r="19" fill="${tone}" stroke="${stroke}" stroke-width="4"/>` +
    ...
    case 'zigzagKink':
      ...
      break;
    case 'candyCane':
      inner = tube('M 0 0 C 34 -8 56 -40 52 -76 C 49 -102 26 -106 20 -90', 21, 14) +
        `<circle cx="20" cy="-90" r="9" fill="#ffffff" stroke="${stroke}" stroke-width="3"/>`;
      break;
    case 'sleekWhip':
    default:
      inner = tube('M 0 0 C 38 -4 62 -34 58 -74 C 55 -102 30 -110 24 -92 C 20 -80 34 -76 40 -86', 21, 14);
      break;
  }
...
function renderHead(dna: CatDNA, colors: CatColors, prefix: string, layout: PoseLayout): string {
  ...
  const foreheadMark = dna.furPattern === 'tabbyStripes'
  ...
  const banditMask = dna.furPattern === 'maskedBandit'
...
function renderEars(dna: CatDNA, colors: CatColors, prefix: string): string {
  ...
  switch (dna.earType) {
    case 'fold':
      left = `<path d="M 184 136 C 174 116 180 94 200 88 C 218 94 226 114 219 130 C 208 139 193 141 184 136 Z" ${outline}/>` +
        `<path d="M 192 116 Q 203 106 213 116" stroke="${stroke}" stroke-width="2.5" fill="none" stroke-linecap="round"/>`;
      break;
    case 'curl':
      left = `<path d="M 178 154 C 166 116 172 80 196 58 C 202 76 200 92 194 106 C 210 96 222 82 226 64 C 236 92 228 126 210 142 C 198 151 186 155 178 154 Z" ${outline}/>` +
        `<path d="M 190 132 C 184 112 188 94 198 80 C 206 94 206 110 202 124 C 198 130 193 132 190 132 Z" ${inner} opacity="0.85"/>`;
      break;
    case 'lynx':
      left = `<path d="M 179 158 C 167 116 171 80 187 60 C 211 74 231 96 240 120 C 226 140 204 152 179 158 Z" ${outline}/>` +
      ...
      break;
    case 'bigServal':
      left = `<path d="M 170 162 C 150 108 156 54 178 28 C 214 50 242 88 250 122 C 232 146 202 158 170 162 Z" ${outline}/>` +
    ...
    case 'roundBear':
      ...
      break;
    case 'floppy':
      left = `<path d="M 182 146 C 156 126 138 138 136 164 C 136 190 156 204 178 197 C 192 190 198 170 197 150 Z" ${outline}/>` +
...
function renderFace(dna: CatDNA, colors: CatColors, prefix: string): string {
  ...
  switch (dna.eyeShape) {
    ...
    case 'sleepyLids':
      ...
      break;
    case 'shockedRound':
      eyes = [[215, gradL, eyeLeftInfo], [285, gradR, eyeRightInfo]].map(([cx, grad, info]) =>
      ...
      break;
    case 'sassySquint':
      eyes = [[215, gradL, -6], [285, gradR, 6]].map(([cx, grad, rot]) =>
    ...
    case 'derpCross':
      ...
      break;
    case 'slitPredator':
      eyes = [[215, gradL], [285, gradR]].map(([cx, grad]) =>
  ...
  switch (dna.mouthEmotion) {
    case 'blep':
      mouth = `<path d="M 236 217 Q 243 224 250 217 Q 257 224 264 217" fill="none" stroke="${stroke}" stroke-width="2.5" stroke-linecap="round"/>` +
    ...
    case 'smugSmile':
      ...
      break;
    case 'gaspO':
      mouth = `<ellipse cx="250" cy="223" rx="8.5" ry="10" fill="#131c2e" stroke="${stroke}" stroke-width="2"/>` +
    ...
    case 'grumpyLine':
      ...
      break;
    case 'yowlScream':
      mouth = `<path d="M 234 216 Q 250 244 266 216 Z" fill="#7f1d34" stroke="${stroke}" stroke-width="2.5" stroke-linejoin="round"/>` +
      ...
      break;
    case 'sleepyZ':
      mouth = `<path d="M 241 219 Q 250 223 259 219" fill="none" stroke="${stroke}" stroke-width="2.5" stroke-linecap="round"/>` +
  ...
  const wx = r1(52 * wl);
  const whiskers = `<g class="cat-whiskers" stroke="${whisker}" stroke-width="1.8" stroke-linecap="round" opacity="0.75" fill="none">` +
  ...
  const whiskerDots = `<g fill="${stroke}" opacity="0.35">` +
  ...
  const butterfly = dna.propItem === 'butterflyOnNose'
...
function renderHeadAccessory(dna: CatDNA, colors: CatColors): string {
  ...
  switch (dna.headAccessory) {
    ...
    case 'fishOnHead':
      ...
      `</g>`;
    case 'flowerCrown':
      return `<g class="acc-flowers">` +
        [[198, 122, '#fda4af', 11], [230, 110, '#f472b6', 13], [270, 110, '#c084fc', 12], [302, 122, '#7dd3fc', 11]].map(([x, y, c, r]) =>
          `<circle cx="${x}" cy="${y}" r="${r}" fill="${c}" stroke="${stroke}" stroke-width="2"/><circle cx="${x}" cy="${y}" r="${(r as number) * 0.35}" fill="#fde047"/>`,
        ).join('') +
      `</g>`;
    case 'frogBeanie':
      return `<g class="acc-frog-beanie">` +
    ...
    case 'partyHat':
      ...
      `</g>`;
    case 'devilHorns':
      return `<g class="acc-devil-horns">` +
...
function renderNeckAccessory(dna: CatDNA, colors: CatColors, layout: PoseLayout): string {
  ...
  switch (dna.neckAccessory) {
    case 'bellCollar':
      return `<g class="acc-bell-collar">` +
    ...
    case 'pearlNecklace': {
      ...
      for (let i = 0; i <= 6; i++) {
        ...
        pearls.push(`<circle cx="${px}" cy="${py}" r="${r1(pr)}" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2"/>`);
      }
      return `<g class="acc-pearls">${pearls.join('')}</g>`;
    }
    case 'bandanaPirate':
      return `<g class="acc-bandana">` +
...
function renderProps(dna: CatDNA, colors: CatColors, layout: PoseLayout, layer: 'behind' | 'front'): string {
  const stroke = colors.lineStroke;
  ...
  switch (dna.propItem) {
    case 'coffeeMug':
      return `<g class="prop-coffee" transform="translate(${rx} ${ry})">` +
      ...
      `</g>`;
    case 'yarnBall':
      return `<g class="prop-yarn" transform="translate(${lx} ${ly})">` +
      ...
      `</g>`;
    case 'laserDot':
      return `<g class="prop-laser" transform="translate(${lx} ${ly})">` +
        `<circle cx="0" cy="0" r="13" fill="#f43f5e" opacity="0.3"/>` +
        `<circle cx="0" cy="0" r="6.5" fill="#ef4444"/>` +
        `<circle cx="0" cy="0" r="2.5" fill="#ffffff"/>` +
      `</g>`;
    case 'mouseFriend':
      return `<g class="prop-mouse" transform="translate(${rx} ${ry})">` +
      ...
      `</g>`;
    case 'fishSkeleton':
      return `<g class="prop-fish-bones" transform="translate(${rx - 30} ${ry - 14})" stroke="#cbd5e1" stroke-linecap="round">` +
      ...
      `</g>`;
    case 'pottedPlant':
      return `<g class="prop-plant" transform="translate(${rx} ${ry})">` +
...
function renderBackdrop(dna: CatDNA, colors: CatColors, prefix: string): string {
  ...
  switch (dna.backdropTheme) {
    case 'sparkleStars':
      return `<g class="backdrop-sparkles" opacity="0.8">` +
        spark(85, 130, 15, '#fbbf24') + spark(420, 105, 12, '#fde047') +
        spark(415, 385, 11, '#fbbf24') + spark(88, 390, 9, '#fde047') +
    ...
    case 'floatingHearts': {
      ...
      return `<g class="backdrop-hearts" opacity="0.7">` +
        heart(80, 140, 17, '#fb7185') + heart(420, 128, 14, '#fda4af') +
        heart(408, 368, 12, '#fecdd3') + heart(96, 372, 11, '#fb7185') +
      `</g>`;
    }
    case 'pawPrints': {
      ...
      return `<g class="backdrop-paws" opacity="0.18" fill="${colors.accent}">` +
        pawPrint(70, 100, -20, 0.9) + pawPrint(420, 120, 25, 1) +
        pawPrint(80, 375, 15, 0.95) + pawPrint(415, 385, -30, 0.9) +
      `</g>`;
    }
    case 'glowingAura':
    ...
    case 'cozyPillow':
      ...
      `</g>`;
    case 'cyberGrid':
      return `<g class="backdrop-cyber" opacity="0.3">` +
    ...
    case 'fishPattern': {
      ...
      return `<g class="backdrop-fish" opacity="0.28" stroke="${colors.accent}" stroke-width="2" fill="none">` +
        fish(66, 118, 1) + fish(388, 138, -1) + fish(90, 372, 1) +
      `</g>`;
    }
    default:
      return '';
