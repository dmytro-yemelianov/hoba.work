...
class McpClient {
  ...
  constructor() {
    this.proc = spawn(
      'npx',
      ['tsx', '--tsconfig', path.join(REPO_ROOT, 'tsconfig.json'), 'packages/mcp/src/index.ts'],
      { cwd: REPO_ROOT }
    );
...
describe('hoba MCP server', () => {
  ...
  it('resolves entity IDs and reports the canonical form of a legacy code', async () => {
    const res = payload(
      await client.request('tools/call', {
        name: 'validate_entity_ids',
        arguments: { ids: ['M-001', 'mech.genuine_technical_skill_shortfall', 'nope.nothing'] },
      })
    );
  ...
  it('refuses a claim the registry does not carry that far', async () => {
    const over = payload(
      await client.request('tools/call', {
        name: 'validate_claim',
        arguments: { id: 'M-001', claim_level: 'proven' },
      })
    );
    ...
    const within = payload(
      await client.request('tools/call', {
        name: 'validate_claim',
        arguments: { id: 'M-001', claim_level: 'observed' },
      })
    );
    expect(within.valid).toBe(true);
    const unknown = await client.request('tools/call', {
      name: 'validate_claim',
      arguments: { id: 'nope.nothing', claim_level: 'observed' },
    });
  ...
  it('exposes the methodology as resources as well as a tool', async () => {
    ...
    const read = await client.request('resources/read', {
      uri: 'hoba://methodology/epistemic-rules',
    });
  ...
  describe('parity with the CLI', () => {
    ...
    it('resolve the same entity from a legacy code, and list the same scenarios', async () => {
      const node = payload(
        await client.request('tools/call', { name: 'get_node', arguments: { id: 'M-001' } })
      );
      expect(node.node.id).toBe(hoba(['get', 'M-001', '--json']).node.id);

      const scenarios = payload(
        await client.request('tools/call', { name: 'get_scenario', arguments: {} })
      );
  ...
  it('explains observations and validates the stage argument', async () => {
    const ok = payload(
      await client.request('tools/call', {
        ...
        arguments: { artifact_ids: ['A-004'], stage: 'technical' },
      })
    );
    ...
    const bad = await client.request('tools/call', {
      name: 'explain_observation',
      arguments: { artifact_ids: ['A-004'], stage: 'bogus' },
    });
    ...
    expect(scenarios.scenarios.map((s: { id: string }) => s.id)).toContain(
    ...
    const one = payload(
      await client.request('tools/call', {
        name: 'get_scenario',
        arguments: { id: 'ghost-refresh' },
      })
    );
    expect(one.scenario.id).toBe('scenario.ghost_refresh');

    const scExplain = payload(
      await client.request('tools/call', {
        name: 'explain_observation',
        arguments: { scenario_id: 'ats-knockout' },
      })
    );
    expect(scExplain.scenario_id).toBe('ats-knockout');
    expect(
      scExplain.analysis.hard_facts.selected_artifacts.map((a: { id: string }) => a.id)
    ).toEqual([
  ...
  it('evaluates temporal anomalies, runway, flow conservation, and pattern emptiness over MCP', async () => {
    const latency = payload(
      await client.request('tools/call', {
        ...
        arguments: {
          process_id: 'proc.the_hiring_funnel_end_to_end',
          from_state: 'recruiter-queue',
          actual_days: 45,
        },
      })
    );
  ...
  it('find_patterns is honest about filtering and traverse_graph honours relation filters', async () => {
    const all = payload(
      await client.request('tools/call', { name: 'find_patterns', arguments: {} })
    );
    expect(all.filter_applied).toBe(false);
    const some = payload(
      await client.request('tools/call', {
        name: 'find_patterns',
        arguments: { artifact_ids: ['A-013'] },
      })
    );
    ...
    const trav = payload(
      await client.request('tools/call', {
        name: 'traverse_graph',
        arguments: {
          start_id: 'mech.genuine_technical_skill_shortfall',
          depth: 1,
          relations: ['operates_at'],
        },
      })
    );
    expect(trav.nodes.map((n: { id: string }) => n.id).sort()).toEqual([
