...
describe('loadRegistryFromDirectory', () => {
  ...
  it('reports schema failures with the offending file path and field', () => {
    const root = writeTempRegistry({
      'data/en/entities/observation/A-001.md': validArtifact.replace(
        'stages: ["ingestion"]',
        'stages: ["nowhere"]'
      ),
    });
    expect(() => loadRegistryFromDirectory(path.join(root, CONTENT_DIRS.en))).toThrow(
      RegistryLoadError
    );
    expect(() => loadRegistryFromDirectory(path.join(root, CONTENT_DIRS.en))).toThrow(
      /A-001\.md.*stages/
  ...
  it('rejects files without frontmatter and a missing manifest', () => {
    const root = writeTempRegistry({
      'data/en/entities/observation/A-001.md': '# no frontmatter\n',
    });
    expect(() => loadRegistryFromDirectory(path.join(root, CONTENT_DIRS.en))).toThrow(
      /frontmatter/
    );
  ...
  it('orders barriers by funnel order regardless of file order', () => {
    ...
    const root = writeTempRegistry({
      'data/en/entities/barrier/B-001.md': b('B-001', 2),
      'data/en/entities/barrier/B-002.md': b('B-002', 1),
    });
    expect(
      loadRegistryFromDirectory(path.join(root, CONTENT_DIRS.en)).barriers.map((x) => x.id)
    ).toEqual(['B-002', 'B-001']);
  });
});
...
describe('findRegistryRoot / loadRegistryFromRoot', () => {
  it('walks upwards to the repository root and loads both mirrors', () => {
    ...
    expect(loadRegistryFromRoot(REPO_ROOT, 'uk').observations.length).toBe(
      loadRegistryFromRoot(REPO_ROOT, 'en').observations.length
    );
  });
});
