  ...
  substrateSchema.parse({
    ...
    eventClasses: [
      { id: 'evc:submit', title: 'Application submitted', emitters: ['cls:applicant'] },
      {
        ...
        emitters: ['cls:recruiter'],
        communicates: true,
      },
    ],
    statements: [{ id: 'sta:s1', about: 'rec:app1', claims: { years: 5 }, fidelity: 'euphemism' }],
    events: [
    ...
    processes: [
      {
        ...
        transitions: [
          { to: 'evc:submit' },
          { from: 'evc:submit', to: 'evc:reject', conditions: ['cnd:funded'] },
        ],
      },
    ],
    cohorts: [
      {
        id: 'coh:c1',
        title: 'This requisition’s applications',
        of: 'cls:application',
        within: 'rec:req1',
      },
    ],
...
describe('substrate schema', () => {
  ...
  it('forces a comparative condition to name its cohort, and an absolute one not to', () => {
    const base = {
      ...
      title: 'x',
      gates: ['evc:submit'],
      owner: { position: 'inside', party: 'rec:r1' },
      determinacy: 'judgement',
      text: 't',
    };
    expect(conditionSchema.safeParse({ ...base, arity: 'comparative' }).success).toBe(false);
    expect(
      conditionSchema.safeParse({ ...base, arity: 'comparative', cohort: 'coh:c1' }).success
    ).toBe(true);
    expect(
      conditionSchema.safeParse({ ...base, arity: 'absolute', cohort: 'coh:c1' }).success
    ).toBe(false);
  });
  ...
  it('forces ownership to be coherent in both directions', () => {
    ...
    expect(
      conditionSchema.safeParse({ ...base, owner: { position: 'ownerless', party: 'rec:r1' } })
        .success
    ).toBe(false);
    expect(conditionSchema.safeParse({ ...base, owner: { position: 'inside' } }).success).toBe(
      false
    );
    expect(conditionSchema.safeParse({ ...base, owner: { position: 'ownerless' } }).success).toBe(
      true
    );
  });
  ...
  it('refuses an amount without evidence — the question-7 policy, in the schema', () => {
    ...
    expect(
      flowSchema.safeParse({ ...flow, amount: { value: 1, unit: 'usd', evidence: [] } }).success
    ).toBe(false);
    expect(
      flowSchema.safeParse({ ...flow, amount: { value: 1, unit: 'usd', evidence: ['EVD-030'] } })
        .success
    ).toBe(true);
...
describe('substrate referential validation', () => {
  it('rejects an emitter that is not a party', () => {
    ...
    expect(
      validateSubstrate(sub)
        .map((p) => p.problem)
        .join(' ')
    ).toContain('not a party');
  });
  ...
  it('rejects a statement on a non-communicating event class', () => {
    ...
    expect(
      validateSubstrate(sub)
        .map((p) => p.problem)
        .join(' ')
    ).toContain('does not communicate');
  });
