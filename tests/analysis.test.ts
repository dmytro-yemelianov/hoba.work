...
const example = () =>
  JSON.parse(
    fs.readFileSync(path.join(REPO_ROOT, 'tests', 'examples', 'rejection-sequence.json'), 'utf-8')
  );
...
describe('validateAnalysis', () => {
  ...
  it('reports a reference that does not resolve against the ontology', () => {
    ...
    const messages = validateAnalysis(a, bundle)
      .map((i) => i.message)
      .join('\n');
    for (const ref of [
      'mech.no_such_mechanism',
      'obs.no_such_observation',
      'int.no_such_intervention',
    ]) {
  ...
  it('leaves the two states that are not points on the scale alone', () => {
    for (const level of ['contradicted', 'unknown']) {
      ...
      expect(
        validateAnalysis(a, bundle).filter((i) => i.rule === 'overclaim'),
        level
      ).toEqual([]);
    }
  });
...
describe('claimRank', () => {
  it('orders the scale weakest to strongest and leaves the two off-scale states unranked', () => {
    const ranked = ['observed', 'compatible', 'supported', 'strongly_supported', 'proven'].map(
      (l) => claimRank(l)!
    );
