...
describe('conformance', () => {
  it('fails a stated minimum only when the arithmetic says so', () => {
    expect(verdicts(checkConformance({ years: 4 }, { required_years: 5 }))['years.short']).toBe(
      'fails'
    );
    expect(verdicts(checkConformance({ years: 5 }, { required_years: 5 }))['years.met']).toBe(
      'passes'
    );
    // No dated history offered: the check does not guess.
    expect(verdicts(checkConformance({}, { required_years: 5 }))['years.unknown']).toBe(
      'undetermined'
    );
  });
  ...
  it('calls a requirement nobody could meet a fact about the posting', () => {
    ...
    expect(report.unsatisfiable[0]!.reason).toEqual({
      code: 'years.impossible',
      params: { required: 8, existed: 5 },
    });
  ...
  it('never turns a missing keyword into a failure', () => {
    ...
    const report = checkConformance(
      { skills: ['go'] },
      { required_skills: ['go', 'kubernetes', 'rust'] }
    );
  ...
  it('treats a band as a ceiling, not a floor', () => {
    expect(
      verdicts(checkConformance({ expectation: 90 }, { band_min: 50, band_max: 80 }))['band.above']
    ).toBe('fails');
    // Under the band is not a gate the process closes on; it is not a pass fail.
    expect(
      verdicts(checkConformance({ expectation: 40 }, { band_min: 50, band_max: 80 }))['band.under']
    ).toBe('passes');
    expect(
      verdicts(checkConformance({ expectation: 60 }, { band_min: 50, band_max: 80 }))['band.inside']
    ).toBe('passes');
    expect(verdicts(checkConformance({ expectation: 60 }, {}))['band.unpublished']).toBe(
      'undetermined'
    );
  });
