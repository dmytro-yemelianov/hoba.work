...
describe('registryContentHash', () => {
  ...
  it('changes when a file is renamed, even though every byte of content is the same', () => {
    ...
    const after = registryContentHash(
      tempRegistry({ ...rest, 'data/en/entities/mechanism/mech.renamed.md': body })
    );
    expect(after).not.toBe(before);
  });
...
describe('the release manifest', () => {
  const manifest = JSON.parse(
    fs.readFileSync(
      path.join(REPO_ROOT, 'apps', 'web', 'public', 'data', 'latest', 'manifest.json'),
      'utf-8'
    )
  );
  ...
  it('reports a semver registry version — DoD 8', () => {
    ...
    expect(loadRegistryFromRoot(resolveRegistryRoot(), 'en').version).toBe(
