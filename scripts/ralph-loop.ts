...
async function runRalphLoop() {
      ...
      score.grade === 'S'
        ...
        : score.grade === 'A'
          ...
          : score.grade === 'B'
            ...
            : '🔴 C';
    process.stdout.write(
      `   • [${badge}] ${preset.name.uk} (${score.total}/100 pts, ${(score.svgLength / 1024).toFixed(1)} KB)\n`
    );
    if (score.total >= 80) presetPassCount++;
  }
  process.stdout.write(
    `   ✓ Presets Grade A/S Rate: ${presetPassCount}/${CAT_PRESETS.length} (${((presetPassCount / CAT_PRESETS.length) * 100).toFixed(0)}%)\n\n`
  );
  ...
  process.stdout.write(
    `   • Grade Distribution: S: ${audit.gradeDistribution.S || 0} | A: ${audit.gradeDistribution.A || 0} | B: ${audit.gradeDistribution.B || 0} | C: ${audit.gradeDistribution.C || 0}\n`
  );
  process.stdout.write(
    `   • Highest Scoring Specimen: "${audit.highestScoring.dna.seed}" (${audit.highestScoring.total}/100 pts)\n`
  );
  process.stdout.write(
    `   • Lowest Scoring Specimen: "${audit.lowestScoring.dna.seed}" (${audit.lowestScoring.total}/100 pts)\n`
  );

  if (audit.lowestScoring.recommendations.length > 0) {
    process.stdout.write(
      `     Recommendations: ${audit.lowestScoring.recommendations.join('; ')}\n`
    );
  }
