---
id: "bar.probation_period_post_start_confirmation"
type: "barrier"
aliases:
  - "B-016"
title: "Probation Period & Post-Start Confirmation"
stage: "post-offer"
order: 16
precedes: []
description: "The post-hire evaluation interval (statutory or contractual probation) during which performance, delivery, and team alignment are formally confirmed."
pass_condition: "The applicable probation interval ends without a valid probation-based termination, and employment continues under the governing contract and law."
specimens:
  -
    kind: "note"
    label: "Probation review milestone"
    lines:
      -
        text: "Day 1–60: Objectives assigned, delivery metrics tracked"
        tell: false
      -
        text: "Day 90: Formal probation sign-off review by engineering manager"
        tell: true
      -
        text: "Status: Probation review completed; employment continues"
        tell: false
    reading: "Completion may be recorded by an explicit review or follow from the interval expiring under the applicable rule; employment did not begin only at this milestone."
perspectives:
  -
    actor: "actor.hiring_manager"
    sees: "Direct on-the-job execution during the first three months, assessing delivery speed, architecture decisions, and team communication."
    reads: "A probation clause may provide a specific evaluation and termination procedure where the governing law and contract allow it."
    does: "Conducts regular 1-on-1 reviews and files the final probation confirmation report."
  -
    actor: "actor.candidate"
    sees: "Initial project onboarding, ramp-up expectations, and milestone checkpoints toward passing probation."
    reads: "Employment already exists; probation may change particular notice or termination rules but does not postpone every employment right."
    does: "Delivers against sprint goals and seeks written confirmation upon completion of the probation interval."
  -
    actor: "actor.employer_policy"
    sees: "Legal compliance boundaries (statutory 3-month caps under the Ukrainian Labour Code, or contractual notice rules under the UK Employment Rights Act 1996)."
    reads: "Only the protections or procedures expressly varied by applicable law or contract change during probation; other rights remain in force."
    does: "Checks that the clause, duration, notice, reasons, and recorded outcome comply with the applicable regime."
status: "active"
evidence_level: "supported"
evidence_ids:
  - "evidence.probation_period_limits_and_dismissal_standards_ukraine_labour_code_art_26_28"
  - "evidence.statutory_notice_and_contractual_probation_parameters_uk_employment_rights_act_1996"
---

# Probation Period & Post-Start Confirmation

The post-hire evaluation interval (statutory or contractual probation) during which performance, delivery, and team alignment are formally confirmed.

### Pass Condition
The applicable probation interval ends without a valid probation-based termination, and employment continues under the governing contract and law.
