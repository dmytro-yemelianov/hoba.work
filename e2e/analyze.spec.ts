...
const probe = (lang: 'en' | 'uk'): string => {
  ...
  return bundle.observations.find((a) => a.id === 'obs.complete_silence_after_submission')!
    .probes[0]!.action;
};
...
test.describe('analysis wizard', () => {
  test('runs the protocol end to end in English', async ({ page }) => {
    ...
    await expect(page.locator('#barriers-output')).toContainText(
      'bar.technical_screen_live_assessment'
    );
    await page.locator('#step-o [data-goto="b"]').click();
    await expect(page.locator('#mechanisms-output')).toContainText(
      'mech.genuine_technical_skill_shortfall'
    );
    await expect(page.locator('#patterns-output')).toContainText(
  ...
  test.describe('in Ukrainian', () => {
    ...
    test('renders Ukrainian verdicts and probe text', async ({ page }) => {
      await page.goto('/analyze');
      await page
        .locator('input[name="artifacts_selected"][value="obs.complete_silence_after_submission"]')
        .check();
  ...
  test('scenario presets populate inputs and evaluate diagnostics', async ({ page }) => {
    ...
    await expect(
      page.locator(
        'input[name="artifacts_selected"][value="obs.republished_job_posting_with_refreshed_date_and_identical_requirement_body"]'
      )
    ).toBeChecked();
