...
test.describe('reading a pasted rejection', () => {
  test('names the trace a template rejection leaves, and offers to continue with it', async ({ page }) => {
    ...
    await expect(result.getByRole('link', { name: /alignment|aligns|template/i }).first()).toBeVisible();
  ...
  test('says so when the letter names no reason', async ({ page }) => {
    ...
    await expect(page.locator('#paste-result').getByRole('button', { name: /continue/i })).toHaveCount(0);
  });

  test('sends a reader whose case is not here to the form, from the analyze page', async ({ page }) => {
    await page.goto('/analyze?lang=en#paste-box');
    await page.locator('#paste-input').fill('the quick brown fox jumps over the lazy dog again and again');
    await page.getByRole('button', { name: /read it/i }).click();
