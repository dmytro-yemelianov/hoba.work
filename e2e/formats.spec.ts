...
test.describe('machine formats', () => {
  ...
  test('an entity is retrievable as Markdown by extension', async ({ request }) => {
    const response = await request.get(
      '/mechanisms/mech.employment_gap_downranking_bias.md?lang=en'
    );
    ...
    expect(body).toContain(
      'canonical: https://hoba.work/mechanisms/mech.employment_gap_downranking_bias'
    );
    expect(body).toContain(
      'json: https://hoba.work/api/v1/mechanisms/mech.employment_gap_downranking_bias.json'
    );
    ...
    expect(body).toMatch(
      /\[bar\.automated_filter_parser_threshold\]\(https:\/\/hoba\.work\/barriers\/bar\.automated_filter_parser_threshold\)/
    );
  });

  test('the canonical URL serves the same document under Accept', async ({ request }) => {
    const byExtension = await (
      await request.get('/mechanisms/mech.employment_gap_downranking_bias.md?lang=en')
    ).text();
    const byHeader = await request.get('/mechanisms/mech.employment_gap_downranking_bias?lang=en', {
  ...
  test('a browser is unaffected', async ({ request }) => {
    const response = await request.get('/mechanisms/mech.employment_gap_downranking_bias', {
  ...
  test('Markdown follows the reader like the page does', async ({ request }) => {
    for (const [locale, expected] of [
      ...
      const response = await request.get('/mechanisms/mech.employment_gap_downranking_bias.md', {
    ...
    const forced = await request.get(
      '/mechanisms/mech.employment_gap_downranking_bias.md?lang=uk',
  ...
  test('every entity type has a Markdown representation', async ({ request }) => {
    for (const path of [
      '/observations/obs.feedback_stating_candidate_is_overqualified_for_the_grade.md',
      '/barriers/bar.headcount_executive_budget_approval.md',
      '/mechanisms/mech.automated_application_expiration_timeout.md',
      '/patterns/pat.closed_then_reposted_requisition_motif.md',
      '/loops/loop.employment_gap_penalty_loop.md',
      '/interventions/int.upfront_compensation_band_disclosure.md',
    ]) {
  ...
  test('the whole catalogue is one file', async ({ request }) => {
    ...
    expect(
      (body.match(/^- \[(?:[A-Z]+-\d{3}|[a-z]+\.[a-z0-9_]+)\]/gm) ?? []).length
  ...
  test('the page offers both representations', async ({ page }) => {
    ...
    await expect(
      page.locator('main a[href="/mechanisms/mech.employment_gap_downranking_bias.md"]')
    ).toBeVisible();
    await expect(
      page.locator('main a[href="/api/v1/mechanisms/mech.employment_gap_downranking_bias.json"]')
    ).toBeVisible();
  });
});
