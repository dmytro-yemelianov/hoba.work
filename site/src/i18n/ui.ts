...
const en = {
  ...
  'method.evidence.text1.after': ' records. The kind describes the source (primary, research, reporting, survey, anecdote, illustrative); the level describes how far the claim is carried (observed, compatible, supported, strongly supported, proven), with two states that are not points on that line: contradicted, and unknown for an entry that describes rather than asserts. A citation raises a claim to “supported” only when it addresses the specific mechanism, not the general topic, and “proven” is refused outright unless a primary or research record is cited.',
...
const uk: Record<UIKey, string> = {
  ...
  'method.evidence.text1.after': '. Вид доказу описує джерело (первинне, дослідження, преса, опитування, поодиноке свідчення, ілюстрація); рівень — наскільки твердження несе вагу (спостережено, сумісно, підтверджено, сильно підтверджено, доведено), плюс два стани поза цією шкалою: спростовано і невідомо — для запису, який описує, а не стверджує. Цитата підіймає твердження до «підтверджено» лише тоді, коли джерело стосується конкретного механізму, а не теми загалом, а «доведено» не приймається взагалі без первинного або дослідницького запису.',
